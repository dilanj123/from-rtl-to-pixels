...
git clone "$REPO" "$ROOT/repo" >/dev/null
cd "$ROOT/repo"
git checkout --detach "$REF" >/dev/null
test "$(git rev-parse HEAD)" = "$(git rev-parse "$REF")"
...
python scripts/analysis/check_publication_assets.py
make ppa
