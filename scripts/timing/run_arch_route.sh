...
OUT="$BUILD/route/freq-${FREQ}MHz-seed-${SEED}"
mkdir -p "$OUT"
set +e
nextpnr-ecp5 --45k --package CABGA381 --speed 6 \
  --json "$BUILD/netlist.json" --freq "$FREQ" --seed "$SEED" \
  --timing-allow-fail --lpf-allow-unconstrained \
  --report "$OUT/nextpnr-report.json" --detailed-timing-report \
  --textcfg "$OUT/route.config" > "$OUT/nextpnr.log" 2>&1
RC=$?
set -e
if [ "$RC" -ne 0 ]; then
  echo "ROUTE_STATUS=FAIL" > "$OUT/status.txt"
  echo "TIMING_STATUS=FAIL" >> "$OUT/status.txt"
  exit "$RC"
fi
...
timing_status = "PASS" if summary.get("timing_clean") else "FAIL"
print(f"TIMING_STATUS={timing_status}")
with open(sys.argv[2], "a") as handle:
    handle.write(f"TIMING_STATUS={timing_status}\n")
PY
