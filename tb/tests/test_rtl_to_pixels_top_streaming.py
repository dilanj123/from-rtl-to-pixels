...
class Ready:
 def __init__(self,mode,seed): self.mode=mode; self.r=random.Random(seed); self.used={}
 def get(self,cycle,oi,valid):
  ...
  if self.mode=='metadata': target=valid and (oi==0 or oi%W==W-1); n=self.used.get(oi,0); self.used[oi]=n+1; return 0 if target and n<2 else 1
  if self.mode=='drain': target=valid and oi>=N-D; lim=4 if oi==N-1 else (3 if oi==N-D else 1); n=self.used.get(oi,0); self.used[oi]=n+1; return 0 if target and n<lim else 1
